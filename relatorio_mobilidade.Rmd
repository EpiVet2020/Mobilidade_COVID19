...
mobilidade_google <- fread("https://raw.githubusercontent.com/EpiVet2020/Mobilidade_COVID19/main/google_mobilidade_pt.csv?token=AO4UTASM3LOXNGRARIPC3C27SMHD2")
...
correlacao_google_grafico <- ggplot(correlacao_google, aes(x = Lag, y = Correlacao, color= `Categoria de local`)) +
  ...
  theme(plot.title = element_text(size=10, face = "bold", hjust = 0.5),
        ...
        axis.title.x = element_text(size = 10, margin = margin(t = 20, b = 20)),
        axis.title.y = element_text(size = 10, margin = margin(r = 20, l = 20))) +
...
correlacao_google_grafico_interativo <- girafe(code = print(correlacao_google_grafico),
...
correlacao_google_marco_maio_grafico_interativo <- ggplotly(correlacao_google_marco_maio_grafico, tooltip = "text") %>%
  layout(annotations=list(text="Correlação entre a MR para Diferentes Categorias de Locais e a GR em \nDiferentes Desfasamentos (Lag) de Março a Maio",xref="paper",x=0.5,yref="paper",y=1,yshift=30,showarrow=FALSE, font=list(size=15)))
...
layout_ggplotly(correlacao_google_marco_maio_grafico_interativo)
...
ggplot(relacao_marco_maio_melt, aes(x = MR, y = Growth_Rate, color = `Categoria de Local`)) +
  ...
  theme(plot.title = element_text(size=10, face = "bold", hjust = 0.5),
        ...
        axis.title.y = element_text(size = 10)) +
  ylim(0.8, 1.3) +
  labs(title = "Relação da MR com a GR de Diferentes Categorias para o \nRespetivo Lag Ótimo entre Março e Maio",
       x = "MR para Lag Ótimo",
       y = "GR") +
...
correlacao_google_maio_hoje_grafico_interativo <- ggplotly(correlacao_google_maio_hoje_grafico, tooltip = "text")
